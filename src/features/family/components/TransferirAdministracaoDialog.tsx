...
import type { MembroFamiliaResponse } from '../types/family'

type Props = { integrante: MembroFamiliaResponse; enviando: boolean; erro: string | null; onConfirmar: () => void; onCancelar: () => void }

function gerarCodigo() { return String(Math.floor(1000 + Math.random() * 9000)) }

export function TransferirAdministracaoDialog({ integrante, enviando, erro, onConfirmar, onCancelar }: Props) {
  ...
  return <dialog ref={dialogRef} aria-labelledby="transferir-administracao-titulo" aria-describedby="transferir-administracao-descricao"
    ...
    <div className="space-y-gutter">
      <div><h2 id="transferir-administracao-titulo" className="text-headline-md font-semibold">Transferir administração?</h2>
        <p id="transferir-administracao-descricao" className="mt-1 text-body-md text-foreground-muted">{integrante.nome} passará a ser Administrador(a) desta família. Você passará a ser Membro.</p></div>
      ...
      {mobile ? <div className="space-y-gutter" aria-label="Código de confirmação">
        ...
        <div className="grid grid-cols-3 gap-2 border-t border-foreground/10 pt-gutter">
          ...
          <button type="button" disabled={enviando || codigoInformado.length === 0} onClick={() => setCodigoInformado((atual) => atual.slice(0, -1))} aria-label="Apagar último dígito" className="min-h-touch rounded-control border border-foreground/20 text-label-lg font-semibold hover:bg-foreground/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:opacity-60">Apagar</button>
          ...
          <button type="button" disabled={enviando || codigoInformado.length === 0} onClick={() => setCodigoInformado('')} className="min-h-touch rounded-control border border-foreground/20 text-label-lg font-semibold hover:bg-foreground/5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:opacity-60">Limpar</button>
        </div>
      </div> : <label className="block text-label-lg font-semibold" htmlFor="codigo-transferencia">Código de confirmação
        <input ref={inputRef} id="codigo-transferencia" inputMode="numeric" pattern="[0-9]*" maxLength={4} value={codigoInformado} disabled={enviando}
      ...
      <div className="grid gap-gutter border-t border-foreground/10 pt-gutter sm:grid-cols-2">
        <button type="button" disabled={!podeConfirmar} onClick={onConfirmar} className="min-h-touch rounded-control bg-amber-500 px-page text-label-lg font-semibold text-foreground hover:bg-amber-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:cursor-not-allowed disabled:opacity-60">{enviando ? 'Transferindo...' : 'Confirmar transferência'}</button>
