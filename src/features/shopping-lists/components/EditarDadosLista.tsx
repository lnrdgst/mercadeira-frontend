...
import {
  ...
  type ListaCompraDetalheResponse,
} from "../types/shoppingList";


export function EditarDadosLista({
  familiaId,
  lista,
  onAtualizada,
  onMutacao,
  ...
  async function reconciliar() {
    ...
    try {
      ...
      if ((error as ApiRequestError).status === 401) logout();
        ...
        setErro(
          "Não foi possível atualizar a lista. Atualize antes de tentar salvar novamente.",
        );
    }
  }
  ...
  async function salvar(event: React.FormEvent<HTMLFormElement>) {
    ...
    setErro(null);
    try {
      const response = await atualizarDadosLista(
        auth.token,
        familiaId,
        lista.id,
        {
          ...
          categoria: String(
            data.get("categoria"),
          ) as ListaCompraDetalheResponse["categoria"],
          estabelecimento:
            String(data.get("estabelecimento") || "").trim() ||
            null,
        },
      );
      if (!response.data)
  ...
  const campo =
    "min-h-touch w-full rounded-card border border-foreground/20 bg-background px-gutter text-body-md";
  return (
    <>
      <button
        ref={acionador}
        ...
        className="flex h-11 w-11 items-center justify-center rounded-control border border-primary text-primary transition-colors hover:bg-primary/5"
        aria-label="Editar informações da lista"
      >
        {/* Ícone de lápis nativo em SVG */}
        <svg
          xmlns="http://w3.org"
          width="20"
          height="20"
      ...
      {editando && (
        ...
        >
          ...
          >
            ...
            {!permitido && (
              <p>
                Os dados desta lista estão disponíveis somente
                para leitura.
              </p>
            )}
            <fieldset
              disabled={
                enviando || !permitido || precisaAtualizar
              }
              className="space-y-gutter disabled:opacity-60"
            >
              <div>
                <label htmlFor="editar-lista-nome">
                  Nome da lista
                </label>
                <input
                  ref={nomeRef}
                  id="editar-lista-nome"
                  name="nome"
                  required
                  maxLength={120}
              ...
              <div>
                <label htmlFor="editar-lista-categoria">
                  Categoria
                </label>
                <select
                  id="editar-lista-categoria"
                  name="categoria"
                  required
                  defaultValue={lista.categoria}
                  className={campo}
                >
                  {Object.entries(categoriaCompraLabels).map(
                    ([value, label]) => (
              ...
              <div>
                <label htmlFor="editar-lista-estabelecimento">
                  Estabelecimento (opcional)
                </label>
                <input
                  id="editar-lista-estabelecimento"
                  name="estabelecimento"
                  maxLength={120}
                  defaultValue={lista.estabelecimento || ""}
                  className={campo}
                />
              </div>
            </fieldset>
