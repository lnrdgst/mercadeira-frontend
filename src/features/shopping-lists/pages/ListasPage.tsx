...
function isoDaDataBr(data: string) {
  ...
  return dataUtc.getUTCFullYear() === Number(ano) && dataUtc.getUTCMonth() === Number(mes) - 1 && dataUtc.getUTCDate() === Number(dia) ? iso : undefined
}

function mascararData(valor: string) {
  const digitos = valor.replace(/\D/g, '').slice(0, 8)
  return digitos.length > 4 ? `${digitos.slice(0, 2)}/${digitos.slice(2, 4)}/${digitos.slice(4)}` : digitos.length > 2 ? `${digitos.slice(0, 2)}/${digitos.slice(2)}` : digitos
}

function CampoData({ label, texto, onChange, tentarAplicar }: { label: string; texto: string; onChange: (texto: string) => void; tentarAplicar: boolean }) {
  const invalido = texto.length === 10 && !isoDaDataBr(texto)
  const incompleto = texto.length > 0 && texto.length < 10
  return <label className="space-y-2 text-label-lg font-semibold">{label}<input value={texto} inputMode="numeric" autoComplete="off" placeholder="dd/mm/aaaa" aria-invalid={(invalido || (tentarAplicar && incompleto)) || undefined} onChange={(event) => onChange(mascararData(event.target.value))} className="min-h-touch w-full rounded-card border border-foreground/20 bg-background px-gutter" />{(invalido || (tentarAplicar && incompleto)) && <span className="block text-label-md text-error">Informe uma data válida.</span>}</label>
}
...
export function ListasPage() {
  ...
  const [rascunhosDatas, setRascunhosDatas] = useState({ inicial: '', final: '' })
  ...
  const dataInicialIso = rascunhosDatas.inicial ? isoDaDataBr(rascunhosDatas.inicial) : undefined
  const dataFinalIso = rascunhosDatas.final ? isoDaDataBr(rascunhosDatas.final) : undefined
  const datasTemporariasInvalidas = (!!rascunhosDatas.inicial && !dataInicialIso) || (!!rascunhosDatas.final && !dataFinalIso) || (!!dataInicialIso && !!dataFinalIso && dataInicialIso > dataFinalIso)
  function abrirFiltros() { setFiltrosTemporarios(filtros); setRascunhosDatas({ inicial: formatarDataBr(filtros.criadaDe), final: formatarDataBr(filtros.criadaAte) }); setTentouAplicar(false); setModalFiltrosAberto(true) }
  function aplicarFiltros() { setTentouAplicar(true); if (!datasTemporariasInvalidas) { atualizarFiltros({ ...filtrosTemporarios, criadaDe: dataInicialIso, criadaAte: dataFinalIso }); setModalFiltrosAberto(false) } }
